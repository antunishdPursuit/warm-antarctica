...
export default function Home() {
  ...
  return <main className="map-product"><section className="map-experience" aria-label="Interactive Antarctica map"><InteractiveMap active={active} region={region} layers={storyLayers} storyMode={storyMode} onSelect={selectStop} onStoryStep={moveStory} />
    ...
    <div className="map-actions"><button onClick={toggleJourney} aria-pressed={playing} disabled={storyMode}>{playing ? "Pause" : journeyElapsed > 0 && journeyElapsed < stops.length * STAGE_DURATION ? "Resume journey" : journeyElapsed > 0 ? "Restart journey" : "Play journey"}</button><button onClick={toggleStoryMode} aria-pressed={storyMode}>{storyMode ? "Exit story mode" : "Story mode"}</button><button onClick={() => selectStop("water")}>Reset view</button><details className="layer-controls"><summary>Layers</summary><div className="layer-panel"><p className="eyebrow">Show on map</p><button onClick={() => toggleLayer("warm")} aria-pressed={storyLayers.warm}>Warm-water paths</button><button onClick={() => toggleLayer("ice")} aria-pressed={storyLayers.ice}>Ice-shelf response</button><button onClick={() => toggleLayer("global")} aria-pressed={storyLayers.global}>Global connection</button></div></details></div>
