...
export default function Home() {
  ...
  return <main className="map-product"><section className="map-experience" aria-label="Interactive Antarctica map"><InteractiveMap active={active} region={region} layers={storyLayers} onSelect={selectStop} />
    <header className="map-header"><div className="header-links"><a href="#top">Warm Antarctica</a><span className="project-kicker">Interactive climate learning map</span><button className="sources-trigger" onClick={() => setSourcesOpen(true)} aria-expanded={sourcesOpen} aria-controls="sources-drawer">Sources</button></div></header>
    ...
    {sourcesOpen && <aside className="sources-drawer" id="sources-drawer" aria-label="Project sources"><div className="drawer-heading"><div><p className="eyebrow">Sources</p><h2>What supports each step</h2></div><button onClick={() => setSourcesOpen(false)} aria-label="Close sources">&rarr;</button></div><ol>{stops.map((stop) => <li key={stop.id}><span>{stop.step}</span><div><b>{stop.label}</b><a href={stop.sourceUrl} target="_blank" rel="noreferrer">{stop.sourceName} &#8599;</a></div></li>)}</ol><details className="map-display-note"><summary>Map display note</summary><p>The circular gap at Antarctica&apos;s South Pole is a known globe-map rendering limit. It is not an ice, ocean, or climate-change feature in this experience.</p></details></aside>}
  </section></main>;
}
